...
module.exports = {
  ...
  postLogin: async (req, res) => {
    ...
    if (user && (await bcrypt.compare(req.body.password, user ? user.password : ""))) {
      if (!user.status)
        return res.json({
      ...
      if (user.activationCode != "" && user.activationCode !== undefined)
        return res.json({
          status: res.statusCode,
          success: false,
          message: "Your account has not been activated. Please double-check your email.",
        });
  ...
  postRegister: async (req, res) => {
    ...
    if (user) {
      ...
      const date = new Date();
      newUser.type = 1;
      newUser.activationCode = _.random(0, 1000000) + newUser.email + _.random(0, 1000000);
      ...
      res.json({
        code: res.statusCode,
        success: true,
        message: "Successful account registration. An email has been sent. Please check your inbox.",
      });
    }
  },
  ...
  postForgotPassword: async (req, res) => {
    ...
    if (user) {
      if (!user.status) {
        ...
        ////
        const date = new Date();
        user.forgotPasswordCode = _.random(0, 1000000) + user.email + _.random(0, 1000000);
        await user.save();
