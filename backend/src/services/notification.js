...
module.exports = {
  gradeFinalizeNotification: async (courseId, assignment, studentId, senderId) => {
  ...
  newCommentNotification: async (courseId, receiverId, senderId, senderName, assignment) => {
    const newNotification = new Notification({
      userId: receiverId,
      sender: senderId,
      type: "NEW_COMMENT",
  ...
  markReviewNotification: async (courseId, receiverId, senderId, senderName, assignment, approved) => {
    const newNotification = new Notification({
      userId: receiverId,
      sender: senderId,
      type: "MARK_REVIEW",
      message: `${senderName} đã ${approved ? "đánh giá" : "bỏ đánh giá"} ${assignment.name}`,
      course: courseId,
    });
    await newNotification.save();
    socket.sendNotice(receiverId, newNotification);
  ...
  newGradeReviewNotification: async (course, senderId, senderName, assignment) => {
    if (!course) return;
